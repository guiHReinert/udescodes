...
data Tree = Node Word' [Index] Tree Tree | Leaf
    deriving Show
...
inserTree (is, w) Leaf = Node w [is] Leaf Leaf
inserTree (is, w) (Node wt it left right)
    | w == wt = Node w (insOrd is it) left right
    | w < wt = Node wt it (inserTree (is,w) left) right
    | w > wt = Node wt it left (inserTree (is,w) right)
...
ploTree :: Tree -> IO()
ploTree Leaf = return ()
ploTree (Node w it left right) = do putStr (w++": "++show it++"\n")
                                    ploTree left
                                    ploTree right
